...
public class RadixTreeImpl<T> implements RadixTree<T> {
	...
	/**
	 * Insert a new string key and its value to the tree. Throw an
	 * IllegalStateException if there is a conflict.
	 * 
	 * @param key
	...
	/**
	 * Insert a new string key and its value to the tree. If there is already a
	 * conflicting value at the insertion point, resolve by calling the value
	 * merger function.
	 * 
	 * @param key
	 *            The string key of the object
	 ...
	 *            The value replicator to use to make a duplicate of a value to
	 *            place into nodes inserted for splits. If null, the same
	 *            instance of value will be used. For values like Strings, using
	 *            the same value instance does no harm as the string's contents
	 *            are immutable. A value whose object carries state that does
	 *            change (i.e. when merged,) however, must be duplicated for
	 *            proper function.
	 */
	public void insert(String key, T value, ValueMerger<T> valueMerger,
			ValueReplicator<T> valueReplicator) {
		try {
	...
	public boolean delete(String key) {
		...
		final Visitor<T> visitor = new Visitor<T>() {
			 ...
			 *            The child Node
			 */
			private void mergeNodes(Tree<RadixData<T>> parent,
					Tree<RadixData<T>> child) {
	 ...
	 * @return The value corresponding to the key. null if the key cannot be
	 *         found.
	 */
	 ...
	 * @param valueMerger
	 *            The function to use for conflicts. If null, conflicts cause an
	 *            IllegalStateException to be thrown.
	 * @pram valueReplicator The value replicator to use to make a duplicate of
	 *       a value to place into nodes inserted for splits. If null, the same
	 *       instance of value will be used. For values like Strings, using the
	 *       same value instance does no harm as the string's contents are
	 *       immutable. A value whose object carries state that does change
	 *       (i.e. when merged,) however, must be duplicated for proper
	 *       function.
	 */
	private void insert(String key, Tree<RadixData<T>> node, T value,
			ValueMerger<T> valueMerger, ValueReplicator<T> valueReplicator) {
		int i = 0;
		...
		if ("".equals(nodeKey) || i == 0 || (i < keylen && i >= nodelen)) {
			...
			if (node.hasChildren()) {
				for (Tree<RadixData<T>> child : node.getChildren()) {
					if (child.getData().getKey()
							.startsWith(newText.charAt(0) + "")) {
						flag = true;
						insert(newText, child, value, valueMerger,
								valueReplicator);
						break;
					}
				}
		...
		// is a prefix of the current node key
			...
			Tree<RadixData<T>> n1 = new Tree<RadixData<T>>(new RadixData<T>(
					nodeKey.substring(i, nodelen), nodeData.isReal(),
					nodeData.getValue()));
			...
			if (i < keylen) {
				node.addChild(new RadixData<T>(key.substring(i, keylen), true,
						value));

				if (valueReplicator != null) {
					nodeData.setValue(valueReplicator.replicate(nodeData
							.getValue()));
				}
		...
		// this key need to be added as the child of the current node
			...
			node.addChild(new RadixData<T>(nodeKey.substring(i, nodelen),
					nodeData.isReal(), nodeData.getValue()));
	...
	private void visit(String prefix, Visitor<T> visitor,
		...
		if (i == keylen && i == nodelen) {
			visitor.visit(prefix, node);
				...
				(i < keylen && i >= nodelen)) { // OR we need to traverse the
												// childern
			if (node.hasChildren()) {
