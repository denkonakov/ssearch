 ...
 */
package semanticdiscoverytoolkit;
 ...
 * <p>
 * A generalized suffix tree builds a tree of a set of strings and all of their
 * substrings. It can be used to find the longest common substrings in O(N)
 * where N is the sum of the lengths of the strings.
 * 
...
public class GeneralizedSuffixTree {
	...
	private static final ValueMerger<SuffixData> VALUE_MERGER = new ValueMerger<SuffixData>() {
		public void merge(RadixData<SuffixData> existingData,
				SuffixData newValue) {
	...
	public GeneralizedSuffixTree(String[] strings, String eosMarker) {
		...
		for (int i = 0; i < strings.length; ++i) {
			...
			for (int j = 0; j < len; ++j) {
				final String suffix = (j == 0) ? string + eosMarker + i
						: string.substring(j) + eosMarker;
				suffixTree.insert(suffix, new SuffixData(i, j), VALUE_MERGER,
						VALUE_REPLICATOR);
			}
		}
	}
	...
	public final class SuffixData {
		...
		private List<SuffixData> shared; // other data's sharing space with this
											// node.
		...
		public SuffixData replicate() {
			final SuffixData result = new SuffixData(stringIndex,
					substrStartIndex);
		...
		public String getString(int depth) {
			return strings[stringIndex].substring(substrStartIndex,
					substrStartIndex + depth);
		}

		public String getSuffix() {
			return substrStartIndex == 0 ? strings[stringIndex]
					: strings[stringIndex].substring(substrStartIndex);
		}

		public void incorporate(SuffixData other) {
			if (shared == null)
				shared = new ArrayList<SuffixData>();
			shared.add(other);
	...
	private final class PathContainer {
		...
		public Set<String> collectLongest(int minSize) {
			...
			for (Path path : paths)
				path.collectLongest(participants2pathElts);

			// now we want to have each participant's best contribution from the
			// largest group
			// map each participant to its longest strings with the most
			// contributors, next most, ... least.
			// define "best" as that with the most contributors and size >=
			// minSize

			final Map<Integer, ContributionLevels> participant2clevels = createContributionLevels(participants2pathElts);

			for (Map.Entry<Integer, ContributionLevels> entry : participant2clevels
					.entrySet()) {
		...
		private final Map<Integer, ContributionLevels> createContributionLevels(
				Map<BitSet, PathElementSet> participants2pathElts) {
			final Map<Integer, ContributionLevels> result = new HashMap<Integer, ContributionLevels>();

			for (Map.Entry<BitSet, PathElementSet> entry : participants2pathElts
					.entrySet()) {
				...
				for (int participant = participants.nextSetBit(0); participant >= 0; participant = participants
						.nextSetBit(participant + 1)) {
					updateContributionLevels(participant, numParticipants,
							pathEltSet, result);
				}
			}
		...
		private final void updateContributionLevels(int participant,
				int numParticipants, PathElementSet pathEltSet,
				Map<Integer, ContributionLevels> p2cl) {
			ContributionLevels clevels = p2cl.get(participant);
			if (clevels == null) {
				clevels = new ContributionLevels(participant, numParticipants,
						pathEltSet);
	...
	private final class ContributionLevels {
		...
		ContributionLevels(int participant, int numParticipants,
				PathElementSet pathEltSet) {
		...
		public void getBestKeys(int minSize, Set<String> results) {
			...
			//
			// get the first level's keys at or above minSize with the most
			// participants
			//

			final List<Integer> ordered = new ArrayList<Integer>(
					num2level.keySet());
	...
	private final class ContributionLevel {
		...
		final void update(PathElementSet pathEltSet) {
			...
			if (this.pathEltSet == null
					|| this.pathEltSet.getKeyLen() < pathEltSet.getKeyLen()) {
	...
	private final class Path {
		 ...
		 * Construct this path.
		 */
		Path(Tree<RadixData<SuffixData>> leaf,
				Map<Tree<RadixData<SuffixData>>, PathElement> node2pathElt) {
			this.pathElements = new LinkedList<PathElement>();

			// create the path from leaf to root
			PathElement childElt = null; // the path element of the child of the
											// current node.
			for (Tree<RadixData<SuffixData>> node = leaf; node != null; node = node
					.getParent()) {
		...
		public void collectLongest(
			...
			for (int i = pathElements.size() - 1; i >= 0; --i) {
				...
				PathElementSet curMapping = participants2pathElts
						.get(curParticipants);
				if (curMapping == null) {
					participants2pathElts.put(curParticipants,
							new PathElementSet(pathElt));
				} else {
					// do a special add that discards shorter, keeps equal,
					// resets with longer.
	...
	private final class PathElement {
		...
		/**
		 * Get the length of this element's contribution to the key (available
		 * after finalized).
		 */
